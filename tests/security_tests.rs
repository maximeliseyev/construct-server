...
use construct_server::e2e::{
    BundleData, ServerCryptoValidator, SuiteKeyMaterial, UploadableKeyBundle,
};
...
async fn test_message_size_limit() {
    ...
    client
        .send(&construct_server::message::ClientMessage::SendMessage(
            oversized_msg,
        ))
        .await
        .unwrap();
    ...
    match response {
        Some(construct_server::message::ServerMessage::Error(e)) => {
            assert!(
                e.message.contains("MESSAGE_TOO_LARGE") || e.message.contains("exceeds maximum")
            );
        }
...
async fn test_username_validation() {
    ...
    for (username, reason) in invalid_usernames {
        ...
        if let Err(e) = result {
            assert!(
                e.to_string().contains("INVALID_USERNAME") || e.to_string().contains("username"),
                "Error should mention username validation: {}",
                e
            );
        }
    }
...
async fn test_rate_limiting_prevents_spam() {
    ...
    for i in 0..150 {
        // Send 150 messages (should exceed default limit of 100/hour)
        let msg = ChatMessage {
            id: Uuid::new_v4().to_string(),
            from: alice_id.clone(),
            to: alice_id.clone(),
    ...
    assert!(
        ...
        "Rate limiting should be triggered after excessive messages"
    );
}
...
async fn test_message_spoofing_prevented() {
    ...
    match received {
        ...
        _ => panic!(
            "Expected FORBIDDEN error for message spoofing, got: {:?}",
            received
        ),
    }
}
...
async fn test_key_bundle_signature_required() {
    ...
    assert!(
        result.is_err(),
        "Bundle with invalid signature format should be rejected"
    );
    ...
    assert!(
        result.is_err(),
        "Bundle with empty signature should be rejected"
    );
}
