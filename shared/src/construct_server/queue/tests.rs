...
async fn test_delivery_track_user_online() {
    ...
    let mut manager = delivery::DeliveryManager::new(
        &mut client,
        &config,
...
async fn test_delivery_mark_delivered() {
    ...
    assert!(is_delivered_after, "Message should be delivered after marking");
...
async fn test_delivery_register_server_instance() {
    ...
    let exists: bool = client.connection_mut()
        .exists(queue_key)
        .await
...
async fn test_session_revoke_all() {
    ...
    manager.create_session(jti1, user_id, ttl).await.expect("Failed to create session 1");
    manager.create_session(jti2, user_id, ttl).await.expect("Failed to create session 2");
    manager.create_session(jti3, user_id, ttl).await.expect("Failed to create session 3");
    
    // Verify all exist
    assert_eq!(manager.validate_session(jti1).await.unwrap(), Some(user_id.to_string()));
    assert_eq!(manager.validate_session(jti2).await.unwrap(), Some(user_id.to_string()));
    assert_eq!(manager.validate_session(jti3).await.unwrap(), Some(user_id.to_string()));
