...
impl MediaConfig {
    fn from_env() -> Result<Self> {
        let data_dir =
            std::env::var("MEDIA_DATA_DIR").unwrap_or_else(|_| "/data/media".to_string());
...
async fn health_check(State(state): State<AppState>) -> Json<HealthResponse> {
    let storage_available =
        state.config.data_dir.exists() || fs::create_dir_all(&state.config.data_dir).await.is_ok();

    Json(HealthResponse {
        status: if storage_available {
            "ok".into()
        } else {
            "degraded".into()
        },
...
async fn download_media(
    ...
    // SECURITY: Handle response builder errors gracefully
