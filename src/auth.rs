...
impl AuthManager {
    pub fn new(config: &Config) -> Result<Self> {
        ...
        let (current_algorithm, encoding_key, decoding_key, legacy_decoding_key) = if let (
            Some(private_key),
            Some(public_key),
        ) =
            (&config.jwt_private_key, &config.jwt_public_key)
        {
            ...
            if config.jwt_secret.is_empty() {
                anyhow::bail!(
                    "JWT_SECRET is required when not using RS256 keys (JWT_PRIVATE_KEY/JWT_PUBLIC_KEY)"
                );
            }
