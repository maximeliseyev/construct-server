...
use construct_db::{Device, HasDbPool};
    ...
    S: HasDbPool + Send + Sync + 'static,
{
    ...
    async fn from_request(req: Request, state: &Arc<S>) -> Result<Self, Self::Rejection> {
        ...
        let device = construct_db::get_device_by_id(state.db_pool(), &device_id)
            .await
            .map_err(|e| {
        ...
        tokio::spawn(async move {
            if let Err(e) =
                construct_db::update_device_last_active(&db_pool, &device_id_clone).await
            {
...
// Blanket impl for any context implementing IntoAppContext is no longer needed —
// all service contexts only need HasDbPool now.
